...
1. **Template Files** (`nodes/ApifyNaverNewsScraper/`) - Blueprint for generated nodes
...
Location: `nodes/ApifyNaverNewsScraper/`

These files serve as the blueprint for all generated nodes:

### Node Definition
- **`ApifyNaverNewsScraper.node.ts`** - Main node class implementing `INodeType`
...
### Properties
- **`ApifyNaverNewsScraper.properties.ts`** - UI field definitions
...
### Metadata
- **`ApifyNaverNewsScraper.node.json`** - Node categories and aliases
...
- Renames `ApifyNaverNewsScraper/` → `ApifyActorName/`
...
generateActorResources() → Convert Apify schema to n8n properties
      ↓
refactorProject() → Rename ApifyNaverNewsScraper → ApifyInstagramScraper
      ↓
...
| File | Purpose |
|------|---------|
| `nodes/ApifyNaverNewsScraper/ApifyNaverNewsScraper.node.ts` | Main node template |
| `nodes/ApifyNaverNewsScraper/helpers/executeActor.ts` | Actor execution logic |
| `nodes/ApifyNaverNewsScraper/helpers/genericFunctions.ts` | API utilities |
...
Edit files in `nodes/ApifyNaverNewsScraper/` - changes will apply to all future generated nodes.
...
Edit `nodes/ApifyNaverNewsScraper/helpers/genericFunctions.ts` → modify `pollRunStatus()` function.

### Add Custom API Headers
Edit `nodes/ApifyNaverNewsScraper/helpers/genericFunctions.ts` → add headers in `apiRequest()` function.
...
- `ApifyNaverNewsScraper.properties.ts` - Completely rebuilt from Actor's input schema
- `properties.json` - Generated from converted schema

**Copied & modified (placeholders replaced):**
- `ApifyNaverNewsScraper.node.ts` - Class names and constants updated
- All helper files - Names and imports updated
- `ApifyNaverNewsScraper.node.json` - Display name updated
